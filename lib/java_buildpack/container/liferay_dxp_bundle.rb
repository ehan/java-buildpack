...
require 'java_buildpack/util/liferay_bundle_start_script'

module JavaBuildpack
  module Container

    # Encapsulates the detect, compile, and release functionality for selecting a `distZip`-like container.
    class LiferayDXPBundle < JavaBuildpack::Component::BaseComponent
      ...
      def compile
        liferay_bundle_start_script(root).chmod 0755
      ...
      def release
        [
          ...
          'exec',
          qualify_path(liferay_bundle_start_script(root), @droplet.root)
      ...
      def augment_app_classpath(content)
        additional_classpath = @droplet.additional_libraries.sort.map do |additional_library|
          "$app_home/#{additional_library.relative_path_from(liferay_bundle_start_script(root).dirname)}"
        end

        update_file liferay_bundle_start_script(root), content,
      ...
      def augment_classpath_content
        content = liferay_bundle_start_script(root).read
